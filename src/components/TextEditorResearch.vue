    ...
    .contents-detail-view
      .contents-section
        ...
        .type-wrapper
          ...
          .type-result-html
            .result-title 에디터 표출 데이터
            .result-wrapper(v-html="htmlParse")
            .result-title 사용자뷰로 전달된 HTML 데이터
            .result-wrapper
              pre.language-css
                .code {{unitData.content}}
            .result-title 서버로 전송되는 데이터 (HTML -> Text)
            .result-wrapper
              .code {{codeParse}}
            .result-title 서버의 Text를 HTML로 변환한 데이터 (Text -> HTML)
            .result-wrapper
              .code {{htmlParse}}
        .type-wrapper
  ...
  data() {
    return {
      usage: "<hw-text-editor \n" +
          ...
          "/>",
      propsData: [
        {name: 'content-data', type:'String(HTML)', default:'', desc: '기본으로 입력되어 있는 데이터 지정 ex.) 게시글 수정'},
