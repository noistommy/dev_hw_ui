<template lang="pug">
  .grid-container
    ...
    .grid-wrapper
      ...
      .ga-grid-2.divide-column-4
        .column(v-for="(item, index) in [1,2]", :key="`divide${index}`")
    .grid-wrapper
      .type-title 기본 그리드 나누기 [2 ~ {{gridLength}}]
      .ga-grid-2(v-for="grid in gridLength" :key="grid" :class="`divide-column-${grid}`")
        .column(v-for="(item, index) in grid", :key="`half${index}`", :data-name="`col-${index + 1}`")
    .grid-wrapper
      .type-title 좌측 우선 나누기 (나머지는 채움) -(적용 불가)
      //.ga-grid-2.left-side(v-for="column in columnCount", :key="column")
        .column(:class="column")
        //.column.two
        .column
    .grid-wrapper
      .type-title 우측 우선 나누기 (나머지는 채움) -(적용 불가)
      //.ga-grid-2.right-side(v-for="column in columnCount", :key="column")
        .column
        //.column.two
        .column(:class="column")
          //span {{column}}
    ...
    .grid-wrapper
      ...
      p 5 + 1 + 3 + 1 + 2 = 12
      .ga-grid-2.row-3.row-md-4.row-sm-6.dense
    ...
    .grid-wrapper.align-rule
      ...
      .ga-grid-2.divide-column-3.row-3.inline(:style="{width: '400px', height: '400px'}", :class="[`justify-${justify}`, `align-${align}`]")
